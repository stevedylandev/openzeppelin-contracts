...
contract $TimelockController is TimelockController {
    ...
    function $onlyRoleOrOpenRole(bytes32 role) external payable onlyRoleOrOpenRole(role) {}

    function $onlyRole(bytes32 role) external payable onlyRole(role) {}

    function $_execute(address target,uint256 value,bytes calldata data) external payable {

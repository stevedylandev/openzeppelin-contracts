...
abstract contract $GovernorTimelockCompound is GovernorTimelockCompound {
    ...
    constructor(string memory name_, ICompoundTimelock timelockAddress) Governor(name_) GovernorTimelockCompound(timelockAddress) payable {
    }

    function $onlyGovernance() external payable onlyGovernance() {}

    function $_queueOperations(uint256 proposalId,address[] calldata targets,uint256[] calldata values,bytes[] calldata calldatas,bytes32 arg4) external payable returns (uint48 ret0) {
        (ret0) = super._queueOperations(proposalId,targets,values,calldatas,arg4);

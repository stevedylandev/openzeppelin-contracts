...
abstract contract $GovernorTimelockAccess is GovernorTimelockAccess {
    ...
    constructor(string memory name_, address manager, uint32 initialBaseDelay) Governor(name_) GovernorTimelockAccess(manager, initialBaseDelay) payable {
    }
    ...
    function $_queueOperations(uint256 proposalId,address[] calldata targets,uint256[] calldata arg2,bytes[] calldata calldatas,bytes32 arg4) external payable returns (uint48 ret0) {
        (ret0) = super._queueOperations(proposalId,targets,arg2,calldatas,arg4);
        emit return$_queueOperations(ret0);
    }

    function $_executeOperations(uint256 proposalId,address[] calldata targets,uint256[] calldata values,bytes[] calldata calldatas,bytes32 arg4) external payable {
        super._executeOperations(proposalId,targets,values,calldatas,arg4);
    }

...
import "../../contracts/mocks/AccessManagedTarget.sol";
...
contract $AccessManagedTarget is AccessManagedTarget {
    bytes32 public constant __hh_exposed_bytecode_marker = "hardhat-exposed";

    constructor(address initialAuthority) AccessManaged(initialAuthority) payable {
    }

    function $restricted() external payable restricted() {}

    function $_setAuthority(address newAuthority) external payable {
        super._setAuthority(newAuthority);
    }

    function $_checkCanCall(address caller,bytes calldata data) external payable {

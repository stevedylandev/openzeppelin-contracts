...
contract $ERC4626FeesMock is ERC4626FeesMock {
    ...
    constructor(string memory name_, string memory symbol_, IERC20 asset_, uint256 entryFeeBasisPoints, address entryFeeRecipient, uint256 exitFeeBasisPoints, address exitFeeRecipient) ERC20(name_, symbol_) ERC4626(asset_) ERC4626FeesMock(entryFeeBasisPoints, entryFeeRecipient, exitFeeBasisPoints, exitFeeRecipient) payable {
    }

    function $_entryFeeBasisPoints() external view returns (uint256 ret0) {
        (ret0) = super._entryFeeBasisPoints();
    }
    ...
    function $_exitFeeBasisPoints() external view returns (uint256 ret0) {
        (ret0) = super._exitFeeBasisPoints();
    }

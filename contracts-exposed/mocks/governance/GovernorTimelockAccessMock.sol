...
import "../../../contracts/mocks/governance/GovernorTimelockAccessMock.sol";
...
import "../../../contracts/governance/extensions/GovernorTimelockAccess.sol";
...
contract $GovernorTimelockAccessMock is GovernorTimelockAccessMock {
    ...
    constructor(string memory name_, uint48 initialVotingDelay, uint32 initialVotingPeriod, uint256 initialProposalThreshold, address manager, uint32 initialBaseDelay, IVotes tokenAddress, uint256 quorumNumeratorValue) Governor(name_) GovernorSettings(initialVotingDelay, initialVotingPeriod, initialProposalThreshold) GovernorTimelockAccess(manager, initialBaseDelay) GovernorVotes(tokenAddress) GovernorVotesQuorumFraction(quorumNumeratorValue) payable {
    }

    function $onlyGovernance() external payable onlyGovernance() {}

    function $_queueOperations(uint256 proposalId,address[] calldata targets,uint256[] calldata values,bytes[] calldata calldatas,bytes32 descriptionHash) external payable returns (uint48 ret0) {
        (ret0) = super._queueOperations(proposalId,targets,values,calldatas,descriptionHash);
        emit return$_queueOperations(ret0);
    }

    function $_executeOperations(uint256 proposalId,address[] calldata targets,uint256[] calldata values,bytes[] calldata calldatas,bytes32 descriptionHash) external payable {
    ...
    function $_setBaseDelaySeconds(uint32 newBaseDelay) external payable {
        super._setBaseDelaySeconds(newBaseDelay);
    }

    function $_setAccessManagerIgnored(address target,bytes4 selector,bool ignored) external payable {
        super._setAccessManagerIgnored(target,selector,ignored);
    }

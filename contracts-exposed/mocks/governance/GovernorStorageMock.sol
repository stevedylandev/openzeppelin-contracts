...
import "../../../contracts/mocks/governance/GovernorStorageMock.sol";
import "../../../contracts/governance/extensions/GovernorStorage.sol";
...
contract $GovernorStorageMock is GovernorStorageMock {
    bytes32 public constant __hh_exposed_bytecode_marker = "hardhat-exposed";

    mapping(uint256 => Checkpoints.Trace208) internal $v_Checkpoints_Trace208;

    event return$_propose(uint256 ret0);
    ...
    constructor(string memory name_, uint48 initialVotingDelay, uint32 initialVotingPeriod, uint256 initialProposalThreshold, TimelockController timelockAddress, IVotes tokenAddress, uint256 quorumNumeratorValue) Governor(name_) GovernorSettings(initialVotingDelay, initialVotingPeriod, initialProposalThreshold) GovernorTimelockControl(timelockAddress) GovernorVotes(tokenAddress) GovernorVotesQuorumFraction(quorumNumeratorValue) payable {
    }

    function $onlyGovernance() external payable onlyGovernance() {}

    function $_propose(address[] calldata targets,uint256[] calldata values,bytes[] calldata calldatas,string calldata description,address proposer) external payable returns (uint256 ret0) {
        (ret0) = super._propose(targets,values,calldatas,description,proposer);
        emit return$_propose(ret0);
    }

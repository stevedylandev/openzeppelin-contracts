...
import "../../../contracts/mocks/governance/GovernorSuperQuorumMock.sol";
...
contract $GovernorSuperQuorumMock is GovernorSuperQuorumMock {
    ...
    constructor(string memory name_, uint48 initialVotingDelay, uint32 initialVotingPeriod, uint256 initialProposalThreshold, IVotes tokenAddress, TimelockController timelockAddress, uint256 quorum_, uint256 superQuorum_) Governor(name_) GovernorSettings(initialVotingDelay, initialVotingPeriod, initialProposalThreshold) GovernorVotes(tokenAddress) GovernorTimelockControl(timelockAddress) GovernorSuperQuorumMock(quorum_, superQuorum_) payable {
    }

...
import "../../../contracts/mocks/governance/GovernorWithParamsMock.sol";
...
contract $GovernorWithParamsMock is GovernorWithParamsMock {
    ...
    function $_getVotes(address account,uint256 blockNumber,bytes calldata params) external view returns (uint256 ret0) {
        (ret0) = super._getVotes(account,blockNumber,params);
    }

    function $_countVote(uint256 proposalId,address account,uint8 support,uint256 weight,bytes calldata params) external payable returns (uint256 ret0) {
        (ret0) = super._countVote(proposalId,account,support,weight,params);

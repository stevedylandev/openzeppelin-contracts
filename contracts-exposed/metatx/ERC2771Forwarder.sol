...
contract $ERC2771Forwarder is ERC2771Forwarder {
    ...
    function $_recoverForwardRequestSigner(ForwardRequestData calldata request) external view returns (bool isValid, address signer) {
        (isValid, signer) = super._recoverForwardRequestSigner(request);
    }

    function $_execute(ForwardRequestData calldata request,bool requireValidRequest) external payable returns (bool success) {
        (success) = super._execute(request,requireValidRequest);
        emit return$_execute(success);
    }

    function $_isTrustedByTarget(address target) external view returns (bool ret0) {
        (ret0) = super._isTrustedByTarget(target);
    }

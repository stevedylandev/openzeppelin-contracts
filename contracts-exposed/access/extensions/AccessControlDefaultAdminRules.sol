// SPDX-License-Identifier: UNLICENSED

pragma solidity >=0.6.0;

import "../../../contracts/access/extensions/AccessControlDefaultAdminRules.sol";
import "../../../contracts/access/AccessControl.sol";
import "../../../contracts/utils/introspection/ERC165.sol";
import "../../../contracts/utils/introspection/IERC165.sol";
import "../../../contracts/interfaces/IERC5313.sol";
import "../../../contracts/access/extensions/IAccessControlDefaultAdminRules.sol";
import "../../../contracts/access/IAccessControl.sol";
import "../../../contracts/utils/Context.sol";
import "../../../contracts/utils/math/SafeCast.sol";
import "../../../contracts/utils/math/Math.sol";
import "../../../contracts/utils/Panic.sol";

contract $AccessControlDefaultAdminRules is AccessControlDefaultAdminRules {
    bytes32 public constant __hh_exposed_bytecode_marker = "hardhat-exposed";

    event return$_grantRole(bool ret0);

    event return$_revokeRole(bool ret0);

    constructor(uint48 initialDelay, address initialDefaultAdmin) AccessControlDefaultAdminRules(initialDelay, initialDefaultAdmin) payable {
    }

    function $onlyRole(bytes32 role) external payable onlyRole(role) {}

    function $_grantRole(bytes32 role,address account) external payable returns (bool ret0) {
        (ret0) = super._grantRole(role,account);
        emit return$_grantRole(ret0);
    }

    function $_revokeRole(bytes32 role,address account) external payable returns (bool ret0) {
        (ret0) = super._revokeRole(role,account);
        emit return$_revokeRole(ret0);
    }

    function $_setRoleAdmin(bytes32 role,bytes32 adminRole) external payable {
        super._setRoleAdmin(role,adminRole);
    }

    function $_beginDefaultAdminTransfer(address newAdmin) external payable {
        super._beginDefaultAdminTransfer(newAdmin);
    }

    function $_cancelDefaultAdminTransfer() external payable {
        super._cancelDefaultAdminTransfer();
    }

    function $_acceptDefaultAdminTransfer() external payable {
        super._acceptDefaultAdminTransfer();
    }

    function $_changeDefaultAdminDelay(uint48 newDelay) external payable {
        super._changeDefaultAdminDelay(newDelay);
    }

    function $_rollbackDefaultAdminDelay() external payable {
        super._rollbackDefaultAdminDelay();
    }

    function $_delayChangeWait(uint48 newDelay) external view returns (uint48 ret0) {
        (ret0) = super._delayChangeWait(newDelay);
    }

    function $_checkRole(bytes32 role) external view {
        super._checkRole(role);
    }

    function $_checkRole(bytes32 role,address account) external view {
        super._checkRole(role,account);
    }

    function $_msgSender() external view returns (address ret0) {
        (ret0) = super._msgSender();
    }

    function $_msgData() external view returns (bytes memory ret0) {
        (ret0) = super._msgData();
    }

    function $_contextSuffixLength() external view returns (uint256 ret0) {
        (ret0) = super._contextSuffixLength();
    }

    receive() external payable {}
}

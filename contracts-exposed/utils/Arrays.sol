...
contract $Arrays {
    ...
    mapping(uint256 => bytes[]) internal $v_bytes_;

    mapping(uint256 => string[]) internal $v_string_;
    ...
    function $unsafeAccess_bytes_(uint256 arr,uint256 pos) external view returns (StorageSlot.BytesSlot memory ret0) {
        (ret0) = Arrays.unsafeAccess($v_bytes_[arr],pos);
    }

    function $unsafeAccess_string_(uint256 arr,uint256 pos) external view returns (StorageSlot.StringSlot memory ret0) {
        (ret0) = Arrays.unsafeAccess($v_string_[arr],pos);
    }
    ...
    function $unsafeMemoryAccess(bytes[] calldata arr,uint256 pos) external pure returns (bytes memory res) {
        (res) = Arrays.unsafeMemoryAccess(arr,pos);
    }

    function $unsafeMemoryAccess(string[] calldata arr,uint256 pos) external pure returns (string memory res) {
        (res) = Arrays.unsafeMemoryAccess(arr,pos);
    }
    ...
    function $unsafeSetLength_bytes_(uint256 array,uint256 len) external payable {
        Arrays.unsafeSetLength($v_bytes_[array],len);
    }

    function $unsafeSetLength_string_(uint256 array,uint256 len) external payable {
        Arrays.unsafeSetLength($v_string_[array],len);
    }

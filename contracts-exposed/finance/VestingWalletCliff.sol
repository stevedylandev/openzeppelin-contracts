...
import "../../contracts/finance/VestingWalletCliff.sol";
...
contract $VestingWalletCliff is VestingWalletCliff {
    bytes32 public constant __hh_exposed_bytecode_marker = "hardhat-exposed";

    constructor(address beneficiary, uint64 startTimestamp, uint64 durationSeconds, uint64 cliffSeconds) VestingWallet(beneficiary, startTimestamp, durationSeconds) VestingWalletCliff(cliffSeconds) payable {
    }

    function $onlyOwner() external payable onlyOwner() {}
    ...
    function $_transferOwnership(address newOwner) external payable {
